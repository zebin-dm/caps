...
from CAPS.network_v4 import CAPSNet
...
class CAPSModel():
    ...
    def __init__(self, args):
        ...
        self.optimizer = optim.AdamW(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.args.lr, weight_decay=1e-3, amsgrad=True)
    ...
    def extract_det_and_des(self, im, src_shape):
        self.model.eval()
        with torch.no_grad():
            coord, descriptor = self.model.exetrct_det_and_des(im, src_shape)
            return coord, descriptor

    def write_summary(self, writer, n_iter):
        # print("%s | Step: %d, Loss: %2.5f" % (self.args.exp_name, n_iter, self.j_loss.item()))
        self.loss_sum_temple.append(self.j_loss.item())

        if n_iter % 100 == 0 and n_iter != 0:
            mean_value = np.mean(self.loss_sum_temple)
            print("%s | Step: %d, Loss: %2.5f" % (self.args.exp_name, n_iter, mean_value))
            self.loss_sum_temple = list()
        ...
        # if True:
            ...
            # modify
            im1_o = cv2.merge([im1_o, im1_o, im1_o])
            im2_o = cv2.merge([im2_o, im2_o, im2_o])
